import type { HubSpotClient as IHubSpotClient, HubSpotPaginatedResponse } from './types.js';
import {
  AuthError,
  NotFoundError,
  ValidationError,
...
export class HubSpotClient implements IHubSpotClient {
  ...
  async request<T>(options: {
      ...
      try {
        ...
        switch (response.status) {
          ...
          default:
            ...
            throw new HubSpotError(errorMessage, 'API_ERROR', response.status);
        }
      } catch (error) {
        ...
        if (error instanceof TypeError && String(error.message).includes('fetch')) {
          throw new HubSpotError(`Network error: ${error.message}`, 'NETWORK_ERROR');
        }
    ...
    throw lastError ?? new HubSpotError('Request failed after retries', 'MAX_RETRIES');
  }
  ...
  async *paginate<T>(options: {
    ...
    while (true) {
      const query = { ...options.query, limit, after: cursor };
      const body = options.body ? { ...options.body as any, limit, after: cursor } : undefined;

      const response = await this.request<HubSpotPaginatedResponse<T>>({
        method: options.method,
        path: options.path,
        query: options.method === 'GET' ? query : options.query,
        body: options.method === 'POST' ? (body ?? query) : undefined,
      });
      ...
      if (!response.paging?.next?.after || !response.results?.length) {
        break;
      }
