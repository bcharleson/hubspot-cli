...
export function output(data: unknown, options: GlobalOptions = {}): void {
  ...
  if (options.fields && typeof data === 'object' && data !== null) {
    ...
    if (Array.isArray(data)) {
      ...
      // Unwrap paginated responses: {results: [...]} → apply to each item
      if (Array.isArray(obj.results)) {
        result = (obj.results as Record<string, unknown>[]).map((item) => pickFields(item, fields));
      } else {
...
function pickFields(
  ...
  for (const field of fields) {
    // Support nested fields like "properties.email"
    if (field.includes('.')) {
